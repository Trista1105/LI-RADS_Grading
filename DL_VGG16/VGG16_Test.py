...
if not os.path.exists(path + '/Prediction/' + model):
    os.mkdir(path + '/Prediction/' + model)
predict_path = path + '/Prediction/' + model + '/' + data_site
...
if __name__ == '__main__':
    ...
    if data_site == '':
        ...
        if img_input == 'img_array':
            Pre_Array = np.load(path + data_subpath + '/Pre_Array.npy')
            A_Array = np.load(path + data_subpath + '/A_Array.npy')
            V_Array = np.load(path + data_subpath + '/V_Array.npy')
            D_Array = np.load(path + data_subpath + '/D_Array.npy')
        if img_input == 'img_array_aug':
            Pre_Array = np.load(path + data_subpath + '/Pre_Array_Aug.npy')
            A_Array = np.load(path + data_subpath + '/A_Array_Aug.npy')
            V_Array = np.load(path + data_subpath + '/V_Array_Aug.npy')
            D_Array = np.load(path + data_subpath + '/D_Array_Aug.npy')
        if img_input == 'patches_array_aug':
            Pre_Array = np.load(path + data_subpath + '/Pre_Array_Patches_Scaled_Aug.npy')
            A_Array = np.load(path + data_subpath + '/A_Array_Patches_Scaled_Aug.npy')
            V_Array = np.load(path + data_subpath + '/V_Array_Patches_Scaled_Aug.npy')
            D_Array = np.load(path + data_subpath + '/D_Array_Patches_Scaled_Aug.npy')
        ...
        if img_input == 'img_array':
            Pre_Array = np.load(path + data_subpath2 + data_site + '/Pre_Array.npy')
            A_Array = np.load(path + data_subpath2 + data_site + '/A_Array.npy')
            V_Array = np.load(path + data_subpath2 + data_site + '/V_Array.npy')
            D_Array = np.load(path + data_subpath2 + data_site + '/D_Array.npy')
        if img_input == 'img_array_aug':
            Pre_Array = np.load(path + data_subpath2 + data_site + '/Pre_Array_Aug.npy')
            A_Array = np.load(path + data_subpath2 + data_site + '/A_Array_Aug.npy')
            V_Array = np.load(path + data_subpath2 + data_site + '/V_Array_Aug.npy')
            D_Array = np.load(path + data_subpath2 + data_site + '/D_Array_Aug.npy')
        if img_input == 'patches_array_aug':
            Pre_Array = np.load(path + data_subpath2 + data_site + '/Pre_Array_Patches_Scaled_Aug.npy')
            A_Array = np.load(path + data_subpath2 + data_site + '/A_Array_Patches_Scaled_Aug.npy')
            V_Array = np.load(path + data_subpath2 + data_site + '/V_Array_Patches_Scaled_Aug.npy')
            D_Array = np.load(path + data_subpath2 + data_site + '/D_Array_Patches_Scaled_Aug.npy')
    ...
    if not home1:
        ...
        adam = tf.keras.optimizers.Adam(lr=1e-4, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)  # 0.001
        ...
        for repeat in range(1, 6):
            for fold in range(1, 6):
                ...
                if data_site == '':
                    ...
                    LIRADS_grade_true_external['Sample_ID'] = LIRADS_grade_true_external['Sample_ID'].astype(str)
                    ...
                    testing_index_ = [int(a) for a in testing_index_]
                ...
                if model == 'AlexNet':
                    if not home1:
                        AlexNet_subpath = '/AlexNet/log/checkpoint'
                    else:
                        AlexNet_subpath = ''

                    if os.path.exists(modelsubpath + AlexNet_subpath + '/best_weights.hdf5'):
                        if not home1:
                            AlexNet_model = load_model(modelsubpath + AlexNet_subpath + '/best_weights.hdf5', custom_objects = {"lr": lr_metric})
                        else:
                            AlexNet_model = AlexNet_parallel(input_shape=(224, 224, 3), num_classes=num_classes)
                            AlexNet_model.load_weights(modelsubpath + AlexNet_subpath + '/best_weights.hdf5')
                        ...
                        print(modelsubpath + AlexNet_subpath + '/best_weights.hdf5', 'Not Exist!!')

                if model == 'VGG16':
                    # if os.path.exists(modelsubpath + '/VGG16/log/vgg16_diameter_bestValAcc_model.h'):
                    #     LIRADS_model = load_model(modelsubpath + '/VGG16/log/vgg16_diameter_bestValAcc_model.h')
                    if not home1:
                        weight_name = 'vgg16_diameter_bestValAcc_weights.h5'
                    else:
                        weight_name = 'vgg16_bestValAcc_weights.h5'

                    if os.path.exists(modelsubpath + model_subpath + '/' + weight_name):
                        LIRADS_model.load_weights(modelsubpath + model_subpath + '/' + weight_name)

                        # AlexNet_model = load_model(modelsubpath + '/AlexNet/log/checkpoint/best_weights.hdf5',
                        ...
                        if data_site == '':
                            ...
                            predict_cls_validation = DataFrame({'LIRADS_grade_predicted': cls_pred_validation})
                            LIRADS_grade_cls_validation_ = DataFrame({'LIRADS_grade': LIRADS_grade_cls_validation})
                            capsule_validation = pd.concat([predict_cls_validation, LIRADS_grade_cls_validation_],
                                                           axis=1)
                        ...
                        if data_site == '':
                            ...
                            capsule_train.to_csv(outputpath + '/training/liradsGrade', sep=',', index=False,
                                                 header=True)
                            capsule_validation.to_csv(outputpath + '/validation/liradsGrade', sep=',', index=False,
                                                      header=True)
                            capsule_testing.to_csv(outputpath + '/testing/liradsGrade', sep=',', index=False,
                                                   header=True)
                        else:
                            capsule_testing.to_csv(outputpath + '/liradsGrade', sep=',', index=False, header=True)

                    else:
                        print(modelsubpath + model_subpath + '/' + weight_name, 'Not Exist!!')